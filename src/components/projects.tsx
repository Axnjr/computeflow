...
export default function Projects({ userProjects }: { userProjects: any[] }) {

    const router = useRouter()
    const sp = useSearchParams()
    const [pros, setPros] = useState(userProjects);

    function showSearchedProject(e: React.ChangeEvent<HTMLInputElement>){
        router.push(`?q=${e.currentTarget.value}`)
        let temp = userProjects.filter((pro) => {
    ...
    return (
        <>
            <div className="flex w-full items-center justify-center gap-2 mt-5 relative">
                ...
                <Button className="h-10" type="submit">+ New project</Button>
            </div>
            ...
            dark:border-neutral-800 px-6 py-8 grid grid-cols-3 gap-4">
                {
                    pros.length > 0 
                        ? 
                    pros.map((project, id) => <div className='bg-neutral-50 dark:bg-neutral-950 h-36 border rounded-xl border-neutral-200
                    dark:border-neutral-800 p-5' key={id}>
                        <div className='flex items-top justify-between'>
                            ...
                            <p className='bg-green-500/20 text-green-500 w-fit h-fit p-1 capitalize rounded-xl text-xs'>{project.status}</p>
                        </div>
                        <br />
                        <div className='text-xs flex items-center justify-between'>
                            <p className='text-neutral-500'>Last deployed: {monthsPassed(project.last_deployed.toString())}</p>
                            <p>{project.region}</p>
                        </div>
                    </div>)
                        :
