...
export default async function ProjectPage({ params }: { params: { project: string } }) {
    ...
    const res: unknown = await xata.db.user_projects.read(params.project)
    if (!res) {
        redirect("/overview")
    } // @ts-ignore
    const project: Project = res[0];
                ...
                <div className="flex space-x-2">
                    <Button variant="outline">{
                        project?.deployed_from.includes("github") 
                            ? 
                        <a href={"https://"+project?.deployed_from+".cheapcloud.dev"}><GitHubLogoIcon className="mr-2 -ml-1"/> Repository</a> 
                            : 
                        <a href={"https://"+project?.deployed_from+".cheapcloud.dev"}>
