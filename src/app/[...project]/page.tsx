...
export default async function ProjectPage({ params, searchParams }: { params: { project: string }, searchParams: { [key: string]: string | string[] | undefined }} ){
    ...
    let res: unknown = await xata.db.user_projects.filter({ id: params.project[0] }).getFirst()
    
    if (res == null) { redirect("/overview") } // @ts-ignore
    let project: ProjectSpecificDataType = {...res};

    if(project.status == "under_deployment"){
        console.log("Project under deployemnt ! fetching IpAddress | Connecting to remote compute")
        let ip = "12:45:89:00" // await getIpAddress(project.instance_metadata.instanceId)
        console.log(ip)
        if(ip != "error"){
        ...
        <main className="h-screen w-full dark:bg-black bg-white dark:text-white text-black">
           <Suspense fallback={<>LOADING ...</>}>
                ...
                dark:bg-neutral-950 flex items-center justify-between">
                    <a href={""} className="text-4xl tracking-tighter capitalize hover:underline group inline-block">
            ...
            {/* <ProjectConfigContext.Provider value={project}>
                {children}
